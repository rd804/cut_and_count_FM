...
batch_size=12
submit_arrays=($(seq 0 $batch_size 17))

for i in ${submit_arrays[@]}; do
    start=$i
    end=$((i+batch_size-1))
    echo "Submitting job for task_id: ${start} to ${end}"
    salloc --qos interactive --time=4:00:00 --nodes=3 --account=m4539 --gres=gpu:4 --constraint=gpu --image rd804/ranode_llf:latest --ntasks-per-node=4 bash window_scan_flow_matching_baseline.sh ${start} ${end}
done
