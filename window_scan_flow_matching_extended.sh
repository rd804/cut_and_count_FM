...
for i in ${task_array[@]}; do
    ...
    dataset=$(awk -v ArrayTaskID=$i '$1==ArrayTaskID {print $5}' $config)
    num_blocks=$(awk -v ArrayTaskID=$i '$1==ArrayTaskID {print $6}' $config)

    wandb_group=window_scan_${dataset}_${n_sig}
    ...
    srun --nodelist=${node} -n 1 -N 1 --exact --gpus-per-task=1 shifter python -u scripts/window_scan_flow_matching.py --n_sig=${n_sig} \
        ...
        --x_train=${x_train} &>./slurm/output/${wandb_group}_${wandb_job_type}_${wandb_run_name}.out &
